...
export async function GET({ url }) {
  ...
  if (url.searchParams.get("researchID") === "null") {
    return new Response(
      JSON.stringify({
        error:
          "No research ID associated with fitbit account. Please try again using the link you received by email.",
      }),
      {
  ...
  try {
    ...
    const { data: update_rsid, error: update_rsid_error } = await supabase
      .from("research_ids")
      .update({
        granted_permission: true,
        granted_permission_at: new Date().toISOString(),
      })
      .eq("research_id", researchID);

    if (update_rsid_error) {
      console.log(update_rsid_error);
    }
    ...
    return new Response(
      JSON.stringify({
        error:
          "There's been a problem. Please try again using the link you received via email. If you still experience issues please email ehsanul.choudhury@gstt.nhs.uk",
      }),
      {
        headers: { "Content-Type": "application/json" },
        status: 400,
